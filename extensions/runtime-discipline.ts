...
export default function runtimeDiscipline(pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", (event: any) => {
    if (!enabled() || (!pendingAllowlist && !pendingEdit)) return
    const now = Date.now()
    if (now - lastFrictionAt < FRICTION_COOLDOWN_MS) return
    const nudges = [pendingAllowlist ? ALLOWLIST_NUDGE : "", pendingEdit ? EDIT_NUDGE : ""].filter(Boolean)
    pendingAllowlist = false; pendingEdit = false; lastFrictionAt = now
