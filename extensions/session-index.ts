...
function summarize(path: string): { filename: string; markdown: string } | null {
	...
	for (const line of readFileSync(path, "utf8").split("\n")) {
		...
		if (r.type === "session") {
			...
			if (m.role === "user") {
				userTurns++
				if (!firstUser) firstUser = clip(textOf(m.content))
	...
	return { filename: `${date}-${slug(name)}.md`, markdown }
}
