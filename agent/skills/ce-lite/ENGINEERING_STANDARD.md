...
5. **Fresh review** — provide a fresh context with contract, evidence matrix, diff/change summary, conventions, and relevant files. It checks correctness, scope, tests, maintainability, compatibility, and security. Resolve blocking findings and repeat affected checks. Mechanical terms must pass `ce_audit` (the extension re-runs the checks). A planted `ce-audit.json` is not evidence.
