...
Load worker skill **`grilling`** when present (not `grill-me` / `grill-with-docs` — those are aliases). Prefer **`domain-modeling`** when terms of art or entity relationships are unstable.
...
When ADRs, CONTEXT.md, glossaries, RFCs, or prior specs exist or were named:

1. Read them first (targeted, lean-ctx — not whole-repo dumps).
2. Resolve request-vs-doc conflicts and true gaps only.
3. Update glossary lines as terms stabilize; ADR drafts only for durable decisions just made.
4. Same completion test as conversational grilling.

This is `grilling` Branch B when that skill is loaded — do not also load `grill-with-docs`.
...
- Park it in the wayfinder map's "Not yet specified" section if it's multi-session (`.scratch/wayfinder/`, via `wayfinding.md` — not the mattpocock `wayfinder` skill).
...
Hand back to ce-lite: write/confirm **contract terms**, then plan (or wayfinding map update). Use `to-spec` only when a formal frozen written spec is needed.
