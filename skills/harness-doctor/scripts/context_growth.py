...
fresh-token growth is uncleared tool outputs vs conversation text.

Baseline 2026-07-30 (253 sessions, 46.6M fresh input tokens):
- toolResult share of context bytes: p50=0.49, p90=0.72
- big (>2KB) tool results reaching the model UNCLEARED: 1835/1860 (98.7%)
- top fresh-token sessions are CONVERSATION-dominated (share 0.24-0.54) —
  turn/message growth management (compaction) is the primary attack there;
  tool-output clearing is the attack for the 88-session mid-tail (6.1M, 13%).

Usage: context_growth.py [--days N] [--json]
"""
...
def main():
    ...
    out = {
        "sessions": len(rows), "total_fresh_M": round(tot / 1000, 1),
    ...
    print(json.dumps(out, indent=2) if args.json else
          f"sessions={out['sessions']} fresh={out['total_fresh_M']}M "
