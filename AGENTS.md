...
When this repo is installed to `~/.pi/agent`, **harness policy SoT is [HARNESS.md](./HARNESS.md)** (skills, tools, extensions, preflight).
`APPEND_SYSTEM.md` is the thin CE-lite trigger strip.
`last30days` is on-demand (not always-on context tokens).

The sections below are the **projects workspace map** used when working across project dirs.
